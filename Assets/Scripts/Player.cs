...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        rb2D.linearVelocity =
            new Vector2(move * speed, rb2D.linearVelocity.y);

        if (move != 0)
        {
            transform.localScale =
                new Vector3(Mathf.Sign(move), 1, 1);
        }
        ...
        {
            rb2D.linearVelocity =
                new Vector2(rb2D.linearVelocity.x, jumpForce);
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.transform.CompareTag("Coin"))
        {
            ...
            // Si consigue 10 monedas, termina el juego
            if (coins >= coinsToWin)
            {
                EndGame();
            }
